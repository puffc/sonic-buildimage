...
class Chassis(ChassisBase):
    ...
    def __initialize_sfp(self):
        ...
        self.num_sfp = len(sfputil_helper.logical)

        from sonic_platform.sfp import Sfp
        for index in range(0, self.num_sfp):
            lanes = [int(x) for x in ports[sfputil_helper.logical[index]]['lanes'].split(',')]
            sfp = Sfp(index, sfputil_helper.logical[index], lanes)
            self._sfp_list.append(sfp)

        for port_num in range(0, self.num_sfp):
            # sfp get uses zero-indexing
    ...
    def get_reboot_cause(self):
        ...
        elif self.get_bmc_reboot_cause(hx_cause):
            reboot_cause = {
    ...
    def set_status_led(self, color):
        ...
        #    reg_file = open(reg_path, "r+")
        #except IOError as e:
        #    print("Error: unable to open file for write: %s" % str(e))
        #    return False
    ...
    def get_bmc_reboot_cause(self, reboot_cause):
        """
    ...
    def get_cpld1_board_rev(self):
        ...
        str = str_dir.get(board_ver, 'Unknown({:#05b})'.format(board_ver))
        return str
